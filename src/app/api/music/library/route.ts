...
export async function GET() {
  ...
  return NextResponse.json({
    tracks: rows.map((row) => ({
      ...row,
      streamable: row.provider === "soundcloud" || row.provider === "audius" || row.provider === "jamendo" || row.provider === "youtube",
...
function parseTrackInput(value: unknown) {
  ...
  if (!body) return null;
  const provider = body.provider === "youtube" || body.provider === "soundcloud" || body.provider === "audius" || body.provider === "jamendo" || body.provider === "musicbrainz" ? body.provider : null;
